...
class OpenCourseServiceImpl extends BaseService implements OpenCourseService
{
    ...
    public function tryManageOpenCourse($courseId)
    {
        ...
        if (!$this->hasOpenCourseManagerRole($courseId, $user['id'])) {
            throw $this->createAccessDeniedException('您不是课程的讲师或管理员，无权操作！');
        }
    ...
    public function setCourseTeachers($courseId, $teachers)
    {
        ...
        foreach (array_values($teachers) as $index => $teacher) {
            if (empty($teacher['id'])) {
                throw $this->createServiceException("讲师ID不能为空，设置课程(#{$courseId})讲师失败");
            }

            $user = $this->getUserService()->getUser($teacher['id']);

            if (empty($user)) {
                throw $this->createServiceException("用户不存在或没有讲师角色，设置课程(#{$courseId})讲师失败");
            }
        ...
        $this->getLogService()->info('open_course', 'update_teacher', "更新课程#{$courseId}的讲师", $teacherMembers);
