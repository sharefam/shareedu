...
class MemberServiceImpl extends BaseService implements MemberService
{
    public function becomeStudentAndCreateOrder($userId, $courseId, $data)
    {
        ...
        if (!$this->getCourseService()->canTakeCourse($course) && !$currentUser->hasPermission('admin_course_manage')) {
    ...
    {
        $course = $this->getCourseService()->getCourse($courseId);
        $this->getCourseService()->canTakeCourse($course);
        $user = $this->getUserService()->getUser($userId);
        if (empty($user)) {
    ...
    public function searchMembers($conditions, $orderBy, $start, $limit, $columns = array())
    {
        ...
        return $this->getMemberDao()->search($conditions, $orderBy, $start, $limit, $columns = array());
    }
    ...
    public function setCourseTeachers($courseId, $teachers)
    {
        ...
        foreach (array_values($teachers) as $index => $teacher) {
            if (empty($teacher['id'])) {
                throw $this->createServiceException("讲师ID不能为空，设置教学计划(#{$courseId})讲师失败");
            }

            $user = $this->getUserService()->getUser($teacher['id']);

            if (empty($user)) {
                throw $this->createServiceException("用户不存在或没有讲师角色，设置教学计划(#{$courseId})讲师失败");
            }
        ...
        $this->getLogService()->info('course', 'update_teacher', "更新教学计划#{$courseId}的讲师", $teacherMembers);

        // 更新教学计划的teacherIds，该字段为教学计划可见讲师的ID列表
    ...
    public function cancelTeacherInAllCourses($userId)
    {
        ...
        $this->getLogService()->info('course', 'cancel_teachers_all', "取消用户#{$userId}所有的教学计划讲师角色");
    }
