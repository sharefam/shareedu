...
class PushMessageEventSubscriber extends EventSubscriber implements EventSubscriberInterface
{
    ...
    public function onArticleCreate(Event $event)
    {
        ...
        if ($this->isIMEnabled()) {
            ...
            $conv = $this->getConversationService()->getConversationByTarget($article['orgId'], 'global');
            $article = $this->convertArticle($article);
            $article['convNo'] = !empty($conv['no']) ? $conv['no'] : '';
    ...
    public function onClassroomJoin(Event $event)
    {
        ...
        if ($this->isIMEnabled() && empty($classroom['batchJoin'])) {
    ...
    protected function getTarget($type, $id)
    {
        ...
        switch ($type) {
            ...
            case 'global':
                ...
                $conv = $this->getConversationService()->getConversationByTarget($id, $type);
                $target['convNo'] = empty($conv) ? $setting['convNo'] : $conv['no'];
                break;
            default:
                // code...
                break;
