...
class ThreadServiceImpl extends BaseService implements ThreadService
{
    ...
    public function deletePost($postId)
    {
        ...
        $memberIds = array($post['userId']);

        $thread = $this->getThreadDao()->get($post['threadId']);
        ...
        try {
            ...
            if (0 == $post['postId']) {
                $posts = $this->getThreadPostDao()->findByPostId($postId);

                $memberIds = array_merge($memberIds, array_keys(ArrayToolkit::index($posts, 'userId')));
                $this->getThreadPostDao()->deleteByPostId($postId);
            }

            $threadPostNum = $this->searchPostsCount(array('threadId' => $post['threadId']));
            $this->getThreadDao()->update($post['threadId'], array('postNum' => $threadPostNum));

            $this->updateGroupAndMemberPostNumByGroupIdAndMemberIds($thread['groupId'], $memberIds);
    ...
    protected function deletePostsByThreadId($threadId)
    {
        $thread = $this->getThreadDao()->get($threadId);

        $posts = $this->getThreadPostDao()->findByThreadId($threadId);
        $memberIds = empty($posts) ? array() : array_keys(ArrayToolkit::index($posts, 'userId'));
    ...
    public function getGroupPostLearnDataForUserLearnDataExtension($conditions)
    {
        $usersGroupPostNumData = $this->getThreadDao()->countGroupPostNumDataByUserIdsAndDate($conditions['userIds'], $conditions['date']);

        $data = array();
        foreach ($usersGroupPostNumData as $userGroupPostNumData) {
            $data[$userGroupPostNumData['userId']] = $userGroupPostNumData['totalGroupPostNum'];
        }
    ...
    private function updateGroupAndMemberPostNumByGroupIdAndMemberIds($groupId, $memberIds)
    {
        $threads = $this->getThreadDao()->findByGroupId($groupId);

        $threadIds = empty($threads) ? array() : array_keys(ArrayToolkit::index($threads, 'id'));

        if ($threadIds) {
            $conditions['threadIds'] = $threadIds;

            $groupPostNum = $this->searchPostsCount($conditions);
            $this->getGroupService()->updateGroup($groupId, array('postNum' => $groupPostNum));
        } else {
            $this->getGroupService()->updateGroup($groupId, array('postNum' => 0));
        }

        if (empty($memberIds)) {
            throw $this->createInvalidArgumentException('MemberIds invalid.');
        }

        $conditions['memberIds'] = $memberIds;

        $countGroupByUserId = $this->getThreadPostDao()->countPostsGroupByUserId($conditions);

        $updateData = array();

        if (empty($countGroupByUserId)) {
            $updateData = array_fill_keys($memberIds, array('postNum' => 0));
        } else {
            $countGroupByUserId = ArrayToolkit::index($countGroupByUserId, 'userId');
            foreach ($memberIds as $id) {
                $updateData[$id] = empty($countGroupByUserId[$id]) ? array('postNum' => 0) : array('postNum' => $countGroupByUserId[$id]['postNum']);
            }
        }

        $this->getThreadMemberDao()->batchUpdate(array_keys($updateData), $updateData, 'userId');
    }
