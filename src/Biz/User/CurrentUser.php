...
class CurrentUser implements AdvancedUserInterface, EquatableInterface, \ArrayAccess, \Serializable
{
    ...
    {
        if (count(array_intersect($this->getRoles(), array('ROLE_TRAINING_ADMIN'))) > 0) {
            return true;
        }
    ...
    public function hasManagePermissionWithOrgCode($orgCode)
    {
        $orgIds = $this->getManageOrgIds();
        if (empty($orgIds) || empty($orgCode)) {
            return false;
        }
        $orgCodeExplode = explode('.', $orgCode);
    ...
    {
        return empty($this->orgIds) ? array($this->rootOrgId) : $this->orgIds;
    }

    public function getOrgCodes()
    {
        return empty($this->orgCodes) ? array($this->rootOrgCode) : $this->orgCodes;
    }

    public function getManageOrgCodes()
    {
        return $this->manageOrgCodes;
    }

    public function getManageOrgIds()
    {
        return $this->manageOrgIds;
    }

    public function getManageOrgIdsRecursively()
    {
        $orgCodes = $this->getManageOrgCodes();
        if (empty($orgCodes)) {
            return array();
        }

        $orgIds = $this->getOrgService()->findOrgsByPrefixOrgCodes($orgCodes, array('id'));
