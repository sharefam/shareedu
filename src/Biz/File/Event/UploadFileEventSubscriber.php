...
class UploadFileEventSubscriber extends EventSubscriber implements EventSubscriberInterface
{
    public static function getSubscribedEvents()
    {
        return array(
            ...
            'course.lesson.materials.delete' => 'onMaterialsDelete',
            'course.materials.delete' => 'onMaterialsDelete',
            'courseSet.materials.delete' => 'onMaterialsDelete',
    ...
    public function onMaterialsDelete(Event $event)
    {
        ...
        $usedCountByFileIds = $this->getCourseMaterialService()->countGroupByFileIds(
            array(
        ...
        array_walk($materials, function ($material) use (&$usedCountByFileIds) {
            $defaultCount = 'coursematerial' === $material['source'] ? 1 : 0;
            $fileId = $material['fileId'];
            $usedCountByFileIds[$fileId] = array(
                'id' => $fileId,
                'usedCount' => empty($usedCountByFileIds[$fileId]['count']) ? $defaultCount : $usedCountByFileIds[$fileId]['count'],
            );
        });

        return $this->getUploadFileService()->batchUpdateFileUsedCount($usedCountByFileIds);
    }
