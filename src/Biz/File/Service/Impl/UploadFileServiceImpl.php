...
class UploadFileServiceImpl extends BaseService implements UploadFileService
{
    ...
    /**
     * @param array $fileUsedCounts
     *                              int id
     *                              int usedCount
     */
    public function batchUpdateFileUsedCount(array $fileUsedCounts)
    {
        if (empty($fileUsedCounts)) {
            return;
        }

        $this->getUploadFileDao()->batchUpdate(array_keys($fileUsedCounts), $fileUsedCounts, 'id');
    }
    ...
    public function getUseFileByFileId($fileId)
    {
        $attachment = $this->getFileUsedDao()->getByFileId($fileId);
        $this->bindFile($attachment);
