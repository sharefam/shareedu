...
class TaskServiceImpl extends BaseService implements TaskService
{
    ...
    public function findTasksFetchActivityByCourseId($courseId)
    {
        /**
         * @Custom 查询task时加入条件
         *         未安装CoMarketingPlugin时，过滤相应的课时(cloudfileshare)
         */
        $conditions = array('courseId' => $courseId);
        if (!$this->biz['pluginConfigurationManager']->isPluginInstalled('CoMarketing')) {
            $conditions['typesNotIn'] = array('cloudfileshare');
        }
