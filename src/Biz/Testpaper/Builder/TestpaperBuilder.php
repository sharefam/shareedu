...
class TestpaperBuilder implements TestpaperBuilderInterface
{
    ...
    public function updateSubmitedResult($resultId, $usedTime, $options = array())
    {
        ...
        if (isset($activity['ext']['finishCondition']['finishScore'])) {
            $fields['passedStatus'] = $fields['score'] >= $activity['ext']['finishCondition']['finishScore'] ? 'passed' : 'unpassed';
        } else {
            $fields['passedStatus'] = 'unpassed';
        }
