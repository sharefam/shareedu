...
class TestpaperServiceImpl extends BaseService implements TestpaperService
{
    ...
    {
        $numInfo = $this->getTestpaperResultDao()->findPaperResultsStatusNumGroupByStatus($testId, $activityId, $userIds);
    ...
    {
        $numInfo = $this->getTestpaperResultDao()->SumPaperResultsStatusNumByCourseIdAndType($courseId, $type);
        if (!$numInfo) {
            return array();
        }
    ...
    public function getNextReviewingResult($courseIds, $activityId, $type, $classroomId = null)
    {
        $conditions = array(
            'courseIds' => $courseIds,
            'lessonId' => $activityId,
