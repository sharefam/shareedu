define(function(require, exports, module) {
    ...
    exports.run = function() {
        ...
        progressBar.on('completed', function() {
            ...
            progressBar.text(Translator.trans('admin.sms_send.send_success'));
            $("#finish-smsSend").show();
        });
    ...
    function exec(url, progressBar, startProgress, endProgress) {
        progressBar.setProgress(startProgress, Translator.trans('admin.sms_send.send_loading'));
        $.ajax(url, {
            async: true,
            type: 'POST'
        }).done(function(data, textStatus, jqXHR) {
            if (data.status == 'error') {
                progressBar.error(makeErrorsText(title + Translator.trans('admin.sms_send.error'), data.errors));
            } else if (typeof(data.index) != "undefined") {
                url = url.replace(/index=\d+/g,'index='+data.index);
                endProgress = data.progress;
                progressBar.setProgress(endProgress, data.message+Translator.trans('admin.sms_send.finish'));
                startProgress = endProgress;
                exec(url, progressBar, startProgress, endProgress);
            } else if (data.status == 'success') {
            	endProgress = 100;
                progressBar.setProgress(endProgress, Translator.trans('admin.sms_send.send_finish'));
            }
        }).fail(function(jqXHR, textStatus, errorThrown) {
            progressBar.error(Translator.trans('admin.sms_send.send_error'));
        });
    }
