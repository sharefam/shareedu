define(function(require, exports, module) {
	...
	exports.run = function() {
            ...
            onFormValidated: function(error, results, $form) {
                ...
                $.post($form.attr('action'), $form.serialize(), function(html){
                    ...
                    if ($table.find( '#' +  $html.attr('id')).length > 0) {
                        ...
                        Notify.success(Translator.trans('admin.tag.update_succcess'));
                    } else {
                        $table.find('tbody').prepend(html);
                        Notify.success(Translator.trans('admin.tag.add_succcess'));
                    }
                    $modal.modal('hide');
				});
        ...
        $modal.find('.delete-tag').on('click', function() {
            if (!confirm(Translator.trans('admin.tag.delete_confirm_message'))) {
                return ;
            }
