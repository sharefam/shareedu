  ...
  var CourseSetClone = Widget.extend({
    ...
    _makeProgressBar: function () {
      return '<div class="modal-dialog ">'
            ...
            +       `<h4 class="modal-title">${Translator.trans('admin.course.copy_course_loading')}`
            +       '</h4>'
            +     '</div>'
    ...
    _doWebClone: function (courseSetId,title) {
      ...
      var intervalId = setInterval(function () {
        progress++;
        if (progress <= 99) {
          progressbar.setProgress(progress, Translator.trans('admin.course.copy_course_progress')+progress+'%');
        }
