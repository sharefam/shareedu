        ...
        var editForm = Widget.extend({
            ...
            _initForm: function() {
                ...
                $(window).on('beforeunload',function(){
                    if ($form.serialize() != $form.data('serialize')) {
                        return Translator.trans('admin.block.visual_edit.beforeunload_confirm_message');
                    }
                });
            ...
            onClickAddBtn: function(e) {
                ...
                if ($panels.length >= $panelGroup.data('count')) {
                    Notify.danger(Translator.trans('admin.block.visual_edit.add_max_message',{panelGroup:$panelGroup.data('count')}));
                } else {
            ...
            onClickRemoveBtn: function(e) {
                if (confirm(Translator.trans('admin.block.visual_edit.delete_confirm_message'))) {
                    var $target = $(e.currentTarget);
                    var $panelGroup = $target.closest('.panel-group');
                    var $parent = $target.closest('.panel.panel-default');
                    var $panels = $panelGroup.children('.panel.panel-default');
                    if ($panels.length == 1) {
                        Notify.danger(Translator.trans('admin.block.visual_edit.empty_message'));
                    } else {
                        $parent.remove();
                        this.refreshIndex($panelGroup);
                    }
                ...
                $element.find('.img-upload').each(function(){
                   ...
                   var uploader = WebUploader.create({
                    ...
                    uploader.on( 'fileQueued', function( file ) {
                        Notify.info(Translator.trans('admin.block.visual_edit.upload_message'), 0);
                        uploader.upload();
                    });

                    uploader.on( 'uploadSuccess', function( file, response ) {
                        self.closest('.form-group').find('input[data-role=img-url]').val(response.url);
                        Notify.success(Translator.trans('admin.block.visual_edit.upload_success'), 1);
                   });

                    uploader.on( 'uploadError', function( file, response ) {
                        Notify.danger(Translator.trans('admin.block.visual_edit.upload_error'));
                    });
        ...
        $("#btn-tabs").sortable({
            ...
            onDrop: function ($item, container, _super, event) {
                ...
                $group.children('.poster-table').each(function(index, object){
                    ...
                    $(this).find('.poster-btn').text(Translator.trans('admin.block.visual_edit.poster',{index:index+1}));
                    $(this).find('input[type=hidden]').val(Translator.trans('admin.block.visual_edit.poster',{index:index+1}));
