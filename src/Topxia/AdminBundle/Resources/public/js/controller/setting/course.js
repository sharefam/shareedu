    ...
    exports.run = function() {
        ...
        if($('#live-course-logo-upload').length>0) {
            ...
            $("#live-course-logo-remove").on('click', function(){
                if (!confirm(Translator.trans('admin.setting.course.live_course_logo_upload.delete_confirm_message'))) return false;
                var $btn = $(this);
                $.post($btn.data('url'), function(){
                    $("#live-course-logo-container").html('');
                    $form.find('[name=live_logo]').val('');
                    $btn.hide();
                    Notify.success(Translator.trans('admin.setting.course.live_course_logo_upload.delete_success'));
                }).error(function(){
                    Notify.danger(Translator.trans('admin.setting.course.live_course_logo_upload.delete_error'));
                });
            });
        }
    };
