...
class UserProcessorImpl extends BaseProcessor implements UserProcessor
{
    ...
    {
        /**
         * 企培版扫码APP登陆，如果用其他工具扫码则跳转到APP下载页.
         **/
        $userAgent = $this->request->headers->get('user-agent');
        if (strpos(strtolower($userAgent), 'edusoho') > -1 or strpos(strtolower($userAgent), 'kuozhi') > -1) {
            ...
            $mobileCode = ((array_key_exists('mobileCode', $result) && !empty($result['mobileCode'])) ? $result['mobileCode'] : 'edusoho-training');

            $params = array('code' => $mobileCode);

            return $this->controller->redirect($this->request->getSchemeAndHttpHost().'/mapi_v2/School/getDownloadUrl?'.http_build_query($params));
        }
    }
    ...
    public function login()
    {
        ...
        $result = $this->validUser($this->request, $username, $password);
    ...
    protected function validUser($request, $username, $password)
    {
        if ($this->isPluginInstalled('LDAP') && $this->getLdapConfigParameter('ldap_login_enabled')) {
            $ldap = $this->getContainer()->get('ldap');

            $result = $this->getLdapService()->authenticate($ldap, $username, $password);

            if ($result['status'] == 'error') {
                return $this->createErrorResponse($result['name'], $result['message']);
            ...
            return $this->loginWithPasswordAndPassword($request, $username, $password);
        }
    }

    protected function loginWithPasswordAndPassword($request, $username, $password)
    {
        $request->request->set('_username', $username);
        $user = $this->loadUserByUsername($request, $username);
        $result = array();
        if (empty($user) || !$this->controller->getUserService()->verifyPassword($user['id'], $password)) {
            $result = array(
        ...
        if (!empty($result)) {
            $result = $this->validateForbidden($request, $result, $username);

            return $this->createErrorResponse($result['error'], $result['message']);
        }

        if ($user['locked']) {
            $result = array(
                'error' => 'user_locked',
                'message' => '用户已锁定，请联系网校管理员'
            );
            return $this->createErrorResponse($result['error'], $result['message']);
        }
    ...
    protected function validateForbidden($request, $result, $username)
    {
        $default = array(
        ...
        if ($forbidden['status'] == 'error') {
            ...
            $failed = $this->controller->getUserService()->markLoginFailed($forbidden['user'] ? $forbidden['user']['id'] : 0, $request->getClientIp());

            if ($forbidden['user']) {
                if ($failed['ipFaildCount'] >= $setting['ip_temporary_lock_allowed_times']) {
                    $result = array(
                        'error' => 'user_locked',
                        'message' => sprintf('当前IP下帐号或密码输入错误过多，请在%s分钟后再试。', $setting['temporary_lock_minutes'])
                    );
                } elseif ($failed['leftFailedCount']) {
                    $result = array(
                        'error' => 'user_locked',
                        'message' => sprintf('帐号或密码错误，您还有%s次输入机会', $failed['leftFailedCount'])
                    );
                } else {
                    $result = array(
                        'error' => 'user_locked',
                        'message' => sprintf('帐号或密码输入错误过多，请在%s分钟后再试，您可以通过找回并重置密码来解除封禁。', $setting['temporary_lock_minutes'])
                    );
                }
    ...
    protected function getLdapConfigParameter($name)
    {
    ...
    {
        return $this->getContainer()->get('kernel')->getPluginConfigurationManager()->isPluginInstalled($pluginName);
    }
    ...
    {
        return ServiceKernel::instance()->createService('LDAPPlugin:LDAP:LdapService');
    }
