...
class SimpleValidator
{
    ...
    {
        $option = array_merge(
            array('minLength' => 4, 'maxLength' => 64),
            $option
        );
    ...
    {
        return (bool) preg_match('/^(?=[^\s]+)((?=.*?[0-9])(?=.*?[a-z])|(?=.*?[0-9])(?=.*?[A-Z])|(?=.*?[A-Z])(?=.*?[a-z])).[\S]{7,19}$/', $value);
    }
    ...
    {
        $option = array_merge(
            array('minLength' => 4, 'maxLength' => 200),
            $option
        );
        ...
        return (bool) preg_match('/^[{\s}\x{4e00}-\x{9fa5}a-zA-z_.·]+$/u', $value);
    }
