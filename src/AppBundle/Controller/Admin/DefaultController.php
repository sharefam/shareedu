...
class DefaultController extends BaseController
{
    ...
    public function feedbackAction(Request $request)
    {
        ...
        $site = array(
            ...
            'siteUserName' => $user->nickname,
            'siteMobile' => $userProfile['mobile'],
            'siteEmail' => $user['email'],
            'cloudSetting' => $cloudSetting,
            'levelName' => empty($info['levelName']) ? '' : $info['levelName'],
        );
        $site = urlencode(http_build_query($site));

        return $this->redirect('http://ct.edusoho.com/question?site='.$site.'');
    }
    ...
    private function domainInspect($request)
    {
        ...
        if ($currentHost != str_replace($filter, '', $siteSetting['url'])) {
            return array(
                'status' => 'warning',
                'errorMessage' => ServiceKernel::instance()->trans('admin.default.message.domain_inspect_error'),
