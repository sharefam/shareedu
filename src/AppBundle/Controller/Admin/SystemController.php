...
class SystemController extends BaseController
{
    ...
    public function emailSendCheckAction()
    {
        ...
        try {
            if (isset($cloudMail['status']) && $cloudMail['status'] === 'enable') {
                return $this->createJsonResponse(array('status' => true, 'message' => ServiceKernel::instance()->trans('admin.system.info.email_send_check.cloud_mail')));
            }
            $mailOptions = array(
                'to' => $user['email'],
            ...
            return $this->createJsonResponse(array('status' => true, 'message' => ServiceKernel::instance()->trans('admin.system.info.email_send_check.success')));
        } catch (\Exception $e) {
            $this->getLogService()->error('system', 'email_send_check', '【系统邮件发送自检】 发送邮件失败：'.$e->getMessage());

            return $this->createJsonResponse(array('status' => false, 'message' => ServiceKernel::instance()->trans('admin.system.info.email_send_check.error')));
        }
    }
    ...
    private function getSystemDiskUsage()
    {
        ...
        $logs = array(
            'name' => '/app/logs',
            'dir' => $rootDir.'/logs',
            'title' => ServiceKernel::instance()->trans('admin.system.info.disk_space_log_tips'),
        );
        ...
        $webFiles = array(
            ...
            'dir' => $webFileDir,
            'title' => ServiceKernel::instance()->trans('admin.system.info.disk_space_files_tips'),
        );
        ...
        $material = array(
            ...
            'dir' => $materialDir,
            'title' => ServiceKernel::instance()->trans('admin.system.info.disk_space_udisk_tips'),
        );
