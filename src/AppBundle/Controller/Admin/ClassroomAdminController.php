...
class ClassroomAdminController extends BaseController
{
    ...
    public function addClassroomAction(Request $request)
    {
        ...
        if (!$user->isSuperAdmin() && !$this->getCurrentUser()->hasPermission('admin_classroom_create')) {
        ...
        if ('POST' == $request->getMethod()) {
            ...
            if (empty($classroom['orgCode']) || !$this->getCurrentUser()->hasManagePermissionWithOrgCode($classroom['orgCode'])) {
                return $this->createMessageResponse('error', $this->trans('admin.manage.org_no_permission'));
            }
            $classroom = $this->getClassroomService()->addClassroom($classroom);
            $this->getResourceVisibleService()->setResourceVisibleScope($classroom['id'], 'classroom', array('showable' => 1, 'publishOrg' => $classroom['orgId']));
