...
class EduCloudController extends BaseController
{
    ...
    protected function createOrgConversation()
    {
        $user = $this->getCurrentUser();
        $targetIds = ArrayToolkit::column($this->getOrgService()->findOrgsByPrefixOrgCode('1.'), 'id');
        $conversations = $this->getConversationService()->searchConversations(array('targetIds' => $targetIds, 'targetType' => 'global'), array(), 0, PHP_INT_MAX);
        $conversations = ArrayToolkit::index($conversations, 'targetId');
        foreach ($targetIds as $targetId) {
            if (!isset($conversations[$targetId])) {
                $this->getConversationService()->createConversation('部门会话', 'global', $targetId, array($user));
            }
        }
    }
    ...
    private function handleUserSmsSetting($dataUserPosted)
    {
        $defaultSetting = array(
            ...
            'sms_activity_signup' => 'on',
            'sms_activity_signup_fail' => 'on',
        );
    ...
    {
        $network = $this->getSettingService()->get('developer');

        if (!empty($network['without_network']) && !$network['without_network']) {
            $api = CloudAPIFactory::create('root');
