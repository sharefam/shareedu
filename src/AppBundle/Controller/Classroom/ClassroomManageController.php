...
class ClassroomManageController extends BaseController
{
    ...
    private function getExportContent($id, $role, $start, $limit, $exportAllowCount)
    {
        ...
        $str = '姓名,用户名,所属部门,岗位,加入学习时间,学习进度';
        ...
        foreach ($classroomMembers as $classroomMember) {
            ...
            $member[] = OrgToolkit::buildOrgsNames($users[$classroomMember['userId']]['orgIds'], $orgs);
            $member[] = empty($posts[$users[$classroomMember['userId']]['postId']]) ? '-' : $posts[$users[$classroomMember['userId']]['postId']]['name'];
    ...
    public function headteacherAction(Request $request, $id)
    {
        ...
        if ($headTeacher) {
            $headTeacherProfile = $this->getUserService()->getUserProfile($headTeacher['id']);
            $newheadTeacher[] = array(
                'id' => $headTeacher['id'],
                'truename' => $headTeacherProfile['truename'],
    ...
    public function assistantsAction(Request $request, $id)
    {
        ...
        foreach ($assistantIds as $key => $assistantId) {
            ...
            $userProfile = $this->getUserService()->getUserProfile($assistantId);
            $sortedAssistants[] = array(
                'id' => $user['id'],
                'truename' => $userProfile['truename'],
                'nickname' => $user['nickname'],
    ...
    public function setInfoAction(Request $request, $id)
    {
        ...
        if ($request->isMethod('POST')) {
            ...
            $data = $this->buildAccessScope($class);
            $this->getResourceVisibleService()->setResourceVisibleScope($id, 'classroom', $class);
            $this->getResourceAccessService()->setResourceAccessScope($id, 'classroom', $data);
    ...
    {
        $trueName = $request->request->get('name');
        $user = array();

        if ('' != $trueName) {
            $user = $this->getUserService()->searchUsers(
                array('truename' => $trueName, 'roles' => 'ROLE_TEACHER'),
                array('createdTime' => 'DESC'),
                0,
                1
            );
    ...
    {
        if (!$this->getClassroomService()->canManageClassroom($id) && !$this->getClassroomService()->canHandleClassroom($id)) {
    ...
    public function resultNextCheckAction($id, $activityId)
    {
        ...
        $checkResult = $this->getTestpaperService()->getNextReviewingResult($courseIds, $activity['id'], $activity['mediaType'], $id);
