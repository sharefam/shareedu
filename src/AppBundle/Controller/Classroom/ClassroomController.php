...
class ClassroomController extends BaseController
{
    ...
    public function headerAction($previewAs, $classroomId)
    {
        ...
        $canAccess = true;
        if ($classroom['conditionalAccess']) {
            $canAccess = $this->getResourceAccessScopeService()->canUserAccessResource('classroom', $classroomId, $user['id']);
        }
        if ($member) {
    ...
    /*
     * 如果用户已购买了此班级，或者用户是该班级的讲师，则显示班级的Dashboard界面。
    ...
    {
        $classroomCourses = $this->getClassroomService()->findClassroomCoursesByClassroomId($classroom['id']);
        $courseIds = empty($classroomCourses) ? array(-1) : ArrayToolkit::column($classroomCourses, 'courseId');
        $learns = $this->getStatusService()->searchStatuses(
            array('classroomCourseIds' => $courseIds, 'classroomId' => $classroom['id']),
    ...
    public function classroomBlockAction($courseId)
    {
        ...
        $classroom = empty($classroomIds) || 0 == count($classroomIds) ? null : $this->getClassroomService()->getClassroom($classroomIds[0]['classroomId']);
    ...
    public function classroomThreadsAction(Request $request, $type)
    {
        ...
        if (!$user->isTeacher()) {
            return $this->createMessageResponse('error', '您不是讲师，不能查看此页面！');
        }
