...
class ThreadController extends CourseBaseController
{
    ...
    public function editAction(Request $request, $courseId, $threadId)
    {
        ...
        if (!($user->isLogin() && $user->id == $thread['userId']) && !($user->hasPermission('admin_course_manage'))) {
            throw $this->createAccessDeniedException();
        }
        ...
        if ('POST' == $request->getMethod()) {
            ...
            if ($form->isValid()) {
                ...
                if ($thread['userId'] != $currentUser->id) {
                    $message = array(
                        'userId' => $currentUser['id'],
                        'userName' => !empty($userProfile['truename']) ? $userProfile['truename'] : $currentUser['nickname'],
                        'courseId' => $courseId,
                        'id' => $threadId,
                        'title' => $thread['title'],
                        'threadType' => $thread['type'],
    ...
    public function editPostAction(Request $request, $courseId, $threadId, $postId)
    {
        ...
        if (!($user->isLogin() && $user->id == $post['userId']) && !($user->hasPermission('admin_course_manage'))) {
