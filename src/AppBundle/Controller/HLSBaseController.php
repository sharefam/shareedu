...
abstract class HLSBaseController extends BaseController
{
    public function playlistAction(Request $request, $id, $token)
    {
        ...
        if ('json' == strtolower($format)) {
            ...
            if ($file['syncId'] > 0) {
                $tokens = $this->getResourcePlatformService()->getResourceToken($file['globalId'], $uri, 'course', $file['belongResourceCodes'], $params);
                $api->setAuthToken($tokens['authToken']);
            }
    ...
    public function clefAction(Request $request, $id, $token)
    {
        ...
        if ($this->isHlsEncryptionPlusEnabled() || !$isMobileUserAgent) {
            $api = CloudAPIFactory::create('leaf');
            $uri = "/hls/clef_plus/{$file[$this->getMediaAttr()][$token['data']['level']]['hlsKey']}";
            if ($file['syncId'] > 0) {
                $tokens = $this->getResourcePlatformService()->getResourceToken($file['globalId'], $uri, 'course', $file['belongResourceCodes'], array());
                $api->setAuthToken($tokens['authToken']);
            }
            $result = $api->get($uri);
    ...
    {
        return $this->createService('LeaseResource:ResourcePlatformService');
    }
}
