        ...
        if ('error' == $forbidden['status']) {
            ...
            if ($forbidden['user']) {
                if ($failed['ipFaildCount'] >= $setting['ip_temporary_lock_allowed_times']) {
                    $message = $this->getServiceKernel()->trans('当前IP下帐号或密码输入错误过多，请在%settingTemporaryLockMinutes%分钟后再试', array('%settingTemporaryLockMinutes%' => $setting['temporary_lock_minutes']));
