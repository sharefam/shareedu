...
class CourseTaskMedia extends AbstractResource
{
    ...
    protected function checkPreview($course, $task)
    {
        ...
        if (empty($task['isFree']) && !$taskCanTryLook) {
            if (!$user->isLogin()) {
                throw new AccessDeniedException('User not login');
            }
            if ($course['parentId'] > 0) {
                throw new AccessDeniedException('You have not join classroom');
            }

            if (!$this->getCourseMemberService()->isCourseMember($course['id'], $user['id'])) {
                throw new AccessDeniedException('The user is not a member of this course');
            }
        }
    ...
    protected function getVideo($course, $task, $activity, $request, $ssl = false)
    {
        ...
        if ('error' === $watchStatus['status']) {
            throw new AccessDeniedException('Video has been viewed for a limited time.');
        }
        ...
        if (empty($file)) {
            throw new ResourceNotFoundException('video', $video['mediaId']);
        }
    ...
    protected function getAudio($course, $task, $activity, $request, $ssl = false)
    {
        ...
        if (empty($file)) {
            throw new ResourceNotFoundException('audio', $audio['mediaId']);
        }
        if (!in_array($file['type'], array('audio', 'video'))) {
            throw new \Exception('Player not supported.');
        }
