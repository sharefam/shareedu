...
class Lesson extends BaseResource
{
    public function get(Application $app, Request $request, $id)
    {
        ...
        if ($task['syncId'] > 0 && $courseSet['deadline'] < time() && in_array($task['type'], array('video', 'audio', 'ppt', 'flash', 'doc'))) {
            return $this->error('lease_expired', '该课程租赁已到期，请联系管理员');
        }
