...
class MemberSync extends BaseResource
{
    ...
    protected function syncOrgConversation($user)
    {
        $orgIds = array();
        ...
        foreach ($user['orgCodes'] as &$orgCode) {
            ...
            $orgIds = array_merge($orgId, $user['orgIds'], $orgIds);
        }
        $orgIds = array_unique($orgIds);

        $convs = $this->getConversationService()->searchConversations(array('targetIds' => $orgIds, 'targetType' => 'global'), array(), 0, PHP_INT_MAX);
        try {
            foreach ($convs as $conv) {
                $convMember = $this->getConversationService()->getMemberByConvNoAndUserId($conv['no'], $user['id']);
                if (!$convMember) {
                    $this->getConversationService()->joinConversation($conv['no'], $user['id']);
                }
            }
        } catch (\Exception $e) {
