define(function(require, exports, module) {
    ...
    var rules = [
        ...
        [
            'check_password',
            /^(?=[^\s]+)((?=.*?[0-9])(?=.*?[a-z])|(?=.*?[0-9])(?=.*?[A-Z])|(?=.*?[A-Z])(?=.*?[a-z])).[\S]{7,19}$/,
            Translator.trans('validate_old.check_password.message')
        ],
        [
            'truename_chinese_alphanumeric',
            /^([\s]|[\u4E00-\uFA29]|[a-zA-Z0-9_.·])*$/i,
            Translator.trans('validate_old.truename_alphanumeric.message', {display: '{{display}}'})
        ],
        [
           'reg_inviteCode',
            /^[a-z0-9A-Z]{5}$/,
