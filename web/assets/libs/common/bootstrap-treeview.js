...
function(a, b, c, d) {
    ...
    g.prototype.clickHandler = function(b) {
        ...
        if (d) {
            var e = c.attr("class") ? c.attr("class").split(" ") : [];
             - 1 !== e.indexOf("expand-icon") ? (this.toggleExpandedState(d, f.options), this.render()) : (-1 !== e.indexOf("check-icon")) && !d.state.disabled ? (this.toggleCheckedState(d, f.options), this.render()) : (d.selectable && !d.state.disabled ? this.toggleSelectedState(d, f.options) : '', this.render())
        }
    },
    ...
    g.prototype.buildTree = function(b, c) {
        if (b) {
            ...
            function(b, e) {
                ...
                return f.append(d.options.enableLinks ? a(d.template.link).attr("href", e.href).append(e.name) : a(d.template.text).html(e.name)),
                d.options.showTags && e.tags && a.each(e.tags,
                function(b, c) {
                    f.append(a(d.template.badge).append(c))
                }),
                d.$wrapper.append(f),
                e.nodes && e.nodes.length && e.state.expanded ? d.buildTree(e.nodes, c) : void 0
            })
        }
    },
    ...
    g.prototype.css = ".treeview .list-group-item{cursor:pointer}.treeview span.indent{margin-left:10px;margin-right:10px}.treeview span.icon{width:16px;margin-right:5px}.treeview .node-disabled{color:silver;cursor:not-allowed}",
...
// 2018-4-21   .treeview span.icon 修改宽度12px改为16px