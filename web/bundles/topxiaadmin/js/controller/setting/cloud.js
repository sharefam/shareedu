define(function(require, exports, module) {
    ...
    exports.run = function() {
        ...
        var uploader = new Uploader({
            ...
            error: function(file) {
                Notify.danger(Translator.trans('admin.setting.cloud.video_watermark_upload_error'));
            },
            success: function(response) {
                ...
                Notify.success(Translator.trans('admin.setting.cloud.video_watermark_upload_success'));
            }
        });

        $("#cloud-video-watermark-remove").on('click', function(){
            if (!confirm(Translator.trans('admin.setting.cloud.video_watermark.delete_confirm_message'))) return false;
            var $btn = $(this);
            $.post($btn.data('url'), function(){
                $("#cloud-video-watermark-container").html('');
                $form.find('[name=video_watermark_image]').val('');
                $btn.hide();
                Notify.success(Translator.trans('admin.setting.cloud.video_watermark.delete_success'));
            }).error(function(){
                Notify.danger(Translator.trans('admin.setting.cloud.video_watermark.delete_error'));
            });
        });
        ...
        var uploader = new Uploader({
            ...
            accept: 'image/png,image/jpg,image/jpeg,imge/bmp,image/gif',
            error: function(file) {
                Notify.danger(Translator.trans('admin.setting.cloud.video_embed_watermark_error'));
            },
            success: function(response) {
                response = $.parseJSON(response);
                ...
                Notify.success(Translator.trans('admin.setting.cloud.video_embed_watermark_success'));
            }
        });
        $("#cloud-video-embed-watermark-remove").on('click', function(){
            if (!confirm(Translator.trans('admin.setting.cloud.video_embed_watermark.delete_confirm_message'))) return false;
            var $btn = $(this);
            $.post($btn.data('url'), function(){
                $("#cloud-video-embed-watermark-container").html('');
                $form.find('[name=video_embed_watermark_image]').val('');
                $btn.hide();
                Notify.success(Translator.trans('admin.setting.cloud.video_embed_watermark.delete_success'));
            }).error(function(){
                Notify.danger(Translator.trans('admin.setting.cloud.video_embed_watermark.delete_error'));
            });
        });
       ...
       if ($('.upload-mode').length >0 ) {
        ...
        validator.addItem({
            ...
            display: Translator.trans('admin.setting.cloud.video_fingerprint_time')
        });
        ...
        $('input[name="video_fingerprint"]').change(function(){
            if($(this).val() == 1) {
                validator.addItem({
                    ...
                    required: true,
                    rule: 'decimal min{min:0.1} max{max:100}',
                    display: Translator.trans('admin.setting.cloud.video_fingerprint')
                });
            } else {
