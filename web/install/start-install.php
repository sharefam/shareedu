...
$twig->addGlobal('edusho_version', \CorporateTrainingBundle\System::CT_VERSION);
...
function install_step3($init_data = 0)
{
    ...
    $biz['autoload.object_maker.service'] = function ($biz) {
        return function ($namespace, $name) use ($biz) {
            ...
            if ('Biz\\' === substr($namespace, 0, strlen('Biz\\'))) {
                $ctNamespace = "CorporateTrainingBundle\\{$namespace}";
    ...
    $biz['autoload.object_maker.dao'] = function ($biz) {
        return function ($namespace, $name) use ($biz) {
            ...
            return new \Codeages\Biz\Framework\Dao\DaoProxy($biz, new $class($biz), $biz['dao.metadata_reader'], $biz['dao.serializer']);
        };
    };

    if ('POST' == strtoupper($_SERVER['REQUEST_METHOD'])) {
        \Biz\Util\FileUtil::emptyDir(\Biz\Util\SystemUtil::getCachePath());
        ...
        $ctinitializer = new \CorporateTrainingBundle\Common\SystemInitializer($output);
        $ctinitializer->setContainer($kernel->getContainer());
        ...
        try {
            ...
            if (empty($init_data)) {
                $ctinitializer->init();
                $admin = $ctinitializer->initAdminUser($_POST);
                _init_setting($admin);
                setcookie('email', $admin['email'], time() + 3600);
                setcookie('mobile', $admin['verifiedMobile'], time() + 3600);
                setcookie('contacts', $_POST['truename'], time() + 3600);

                $ctinitializer->initMagicSetting();
                $ctinitializer->initFolders();
                $ctinitializer->initLockFile();
                $ctinitializer->initRegisterSetting($admin);
                $ctinitializer->initCourseSetting();
                $ctinitializer->initClassroomSetting();
                $ctinitializer->initCorporateTrainingDefaultSetting();
                $ctinitializer->initArea();
                $ctinitializer->initDiscoveryColumn();
                $ctinitializer->initThemeConfig();
                $ctinitializer->initQueueuJob();
            } else {
...
{
    $userAgent = 'System Install Client 1.0';
    $connectTimeout = 10;
    $timeout = 10;
...
function postApiV2Request($url, $params)
{
    $curl = curl_init();
...
function _create_database($config, $replace)
{
    try {
        ...
        if (empty($config['index'])) {
            ...
            $sql = file_get_contents('./train.sql');
            $result = $pdo->exec($sql);
        ...
        if (!empty($config['database_init']) && 1 == $config['database_init']) {
            $index = $config['index'];
...
function _initKey()
{
    ...
    $keys = $applier->applyKey($users[0], 'training', 'install');
...
function _postOfficialWeb()
{
    global $biz;
    $settingService = $biz->service('System:SettingService');
    $settings = $settingService->get('storage');
    $siteInfo = $settingService->get('site');

    if (!empty($settings['cloud_access_key'])) {
        $formData['domainName'] = $_SERVER['HTTP_HOST'];
        $formData['clientIp'] = $_SERVER['REMOTE_ADDR'];
        $formData['contacts'] = empty($_COOKIE['contacts']) ? '' : $_COOKIE['contacts'];
        $formData['mobile'] = empty($_COOKIE['mobile']) ? 0 : $_COOKIE['mobile'];
        $formData['email'] = empty($_COOKIE['email']) ? '' : $_COOKIE['email'];
        $formData['companyName'] = empty($siteInfo['name']) ? '' : $siteInfo['name'];
        $formData['industry'] = '';
        $formData['employeeNumber'] = 0;
        $formData['onlineTime'] = time();
        $formData['accessKey'] = $settings['cloud_access_key'];
        postApiV2Request('http://ct.edusoho.com/api/app_install', json_encode($formData));
    }
}

function _checkWebRoot()
{
    $host = empty($_SERVER['HTTP_REFERER']) ?: $_SERVER['HTTP_REFERER'];
