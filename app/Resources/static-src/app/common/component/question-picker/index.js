...
export default class QuestionPicker {
  constructor($questionPickerBody, $questionAppendForm) {
    this.batchSelectInCookie = new BatchSelectInCookie($questionPickerBody, 'courseSetQuestionIds');
  ...
  pagination(event) {
    ...
    $.get($btn.attr('href'),html=> {
      ...
      new BatchSelectInCookie($('#question-picker-body',window.parent.document), 'courseSetQuestionIds');
    });
    return false;
  }
  ...
  batchSelectSave(event) {
    ...
    let questionIds = this.batchSelectInCookie.itemIds;

    if($('#itemIds').length > 0) {
      questionIds = $('#itemIds').val();
